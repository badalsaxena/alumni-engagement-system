...
export default function StudentChatPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/connections/my`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
      .then(d => {
        const conn = (d.as_student || []).find(c => c.id === connectionId);
        if (conn) setOtherUser(conn.alumni);
      });
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-col h-[calc(100vh-6rem)]" data-testid="chat-page">
        {/* Header */}
        <div className="glass-panel p-4 flex items-center gap-4 mb-4">
          <Link to="/student/connections" className="text-white/40 hover:text-white" data-testid="back-to-connections">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-2">
          {loading ? (
            ...
            <div className="flex items-center justify-center h-full text-white/30 text-sm">No messages yet. Start the conversation!</div>
          ) : (
            messages.map((msg) => {
        ...
        {/* Input */}
        <form onSubmit={sendMessage} className="mt-4 flex gap-2" data-testid="chat-form">
          <input
            value={newMsg}
          ...
          />
          <button type="submit" disabled={!newMsg.trim() || sending} className="btn-primary px-4 disabled:opacity-50" data-testid="chat-send-btn">
